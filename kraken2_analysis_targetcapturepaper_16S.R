...
data <-import_biom("./biom_files_targetcapture/bracken_genera.16S_targetcapture.biom", parseFunction = parse_taxonomy_default)
...
#I added a taxa to this that is likely a false positive from Kraken
data <- subset_taxa(data, (!Kingdom %in% "Eukaryota")) # Mitochondria
data <- subset_taxa(data, (!Kingdom %in% "Nucletmycea")) # Mitochondria
data <- subset_taxa(data, (!Kingdom %in% "Holozoa")) # Mitochondria
data <- subset_taxa(data, (!Family %in% "Mitochondria")) # Mitochondria
...
ideal <- read.csv("mock_community.csv")
...
#######
#
#for_gg_ideal <- bind_rows(for_gg_ideal, data.frame(Type = "Mock community", Genus_lumped = "Other", Family_lumped = "Other", Phylum_lumped = "Other"))
#
#library(dplyr)
...
#	missing_genera$Type <- "Mock community"
...
#	missing_families$Type <- "Mock community"
...
#	missing_phyla$Type <- "Mock community"
...
##################################
# Amplicon mock community
##################################

amplicon <- read.table("./biom_files_amplicon/16S/table.hostremoved-l6.tsv", sep = "\t", header = TRUE, row.names = 1, check.names=FALSE)
taxon_list = c("ZymoMockDNA", "ZymoMockDNA_1685")
mock_community <- subset(amplicon, select = names(amplicon) %in% taxon_list)
OTU = otu_table(mock_community, taxa_are_rows = TRUE)
...
mock_community = phyloseq(OTU, TAX)
...
# adds metadata to data
mock_community@sam_data<-sample_data(samdf)
...
ideal <- read.csv("mock_community_amplicon.csv")
...
names(for_gg_ideal)[1:3] <- c("Phylum_lumped", "Family_lumped", "Genus_lumped")

for_gg %>% mutate(Genus_lumped = ifelse((! Genus %in% ideal$Genus), "Other", as.character(Genus)), Genus_lumped = factor(Genus_lumped)) -> for_gg
# Code to force all factors to show in the following ggplot even with 0 data to avoid messing up legend ordering
all_levels <- union(levels(factor(for_gg$Genus_lumped)), levels(factor(for_gg_ideal$Genus_lumped)))
for_gg$Genus_lumped <- factor(for_gg$Genus_lumped, levels = all_levels)
for_gg_ideal$Genus_lumped <- factor(for_gg_ideal$Genus_lumped, levels = all_levels)
...
# Use guides = collect to enforce common legend and fill colors
mockcommunity_fig <- pct_mockcommunity_genus + pct_mockcommunity_genus_ideal + patchwork::plot_layout(nrow = 1, ncol = 2, guides = "keep") + patchwork::plot_annotation(tag_levels = "A")
...
mockcommunity_fig <- (pct_mockcommunity_family + pct_mockcommunity_family_ideal) + patchwork::plot_layout(nrow = 1, ncol = 2, guides = "keep") + patchwork::plot_annotation(tag_levels = "A")
...
mockcommunity_fig <- (pct_mockcommunity_phylum + pct_mockcommunity_phylum_ideal) + patchwork::plot_layout(nrow = 1, ncol = 2, guides = "keep") + patchwork::plot_annotation(tag_levels = "A")
